...
def _seedInterlockDemo():
	...
	# Ensure Plant Alm_*/Value carry Equality alarms for Alarm Configuration browse/edits.
	...
	try:
		...
		for alm, prio, disp in (
			...
			try:
				cfg = system.tag.getConfiguration(leaf, False)[0]
			except Exception:
				continue
			if cfg.get("alarms"):
				alarmed += 1
		...
		try:
			...
			logger.info("AlarmConfig.listAlarms Main Liq SV rows=%d" % nrows)
		except Exception as le:
			logger.warn("AlarmConfig.listAlarms failed: %s" % str(le))
	except Exception as e:
		logger.warn("Plant valve alarm ensure failed: %s" % str(e))
	...
	try:
		...
		qv = system.tag.readBlocking([
			...
			plantBase + "/Status/Value",
		])
		logger.info(
			"valve cmd self-test CLOSE OpenLS=%s ClosedLS=%s Status=%s q=%s"
		...
		qv2 = system.tag.readBlocking([
			plantBase + "/OpenLS/Value",
			plantBase + "/ClosedLS/Value",
			plantBase + "/OPER/Value",
			plantBase + "/MAINT/Value",
		])
		logger.info(
			"valve cmd self-test OPEN/OPER OpenLS=%s ClosedLS=%s OPER=%s MAINT=%s"
			% (qv2[0].value, qv2[1].value, qv2[2].value, qv2[3].value)
		)
	except Exception as e:
		logger.warn("valve cmd self-test failed: %s" % str(e))
